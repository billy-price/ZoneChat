...
import os
import asyncio
from replit import db as discord_roblox # the database
...
discord_log_handler = logging.FileHandler(filename='discord.log', encoding='utf-8', mode='a')
...
async def unregister(ctx):
  ...
  for roblox_name, discord_id in discord_roblox.items():
    if discord_id == ctx.author.id:
      names.append(roblox_name)
      del discord_roblox[roblox_name]

  if len(names) == 0:
    await ctx.send('No roblox usernames registered with your discord ID')

  await ctx.send(f'Unregistered `{", ".join(names)}` from your discord ID')
...
async def move(roblox_name, zone_name):
  ...
  if user_id is None:
    print(f"Discord ID of {roblox_name} not registered.")
    return
  ...
  if user.voice is None:
    ...
    try:
      print(f"{user.name} ({roblox_name}): {user.voice.channel.name or ''} -> {zone_name}")
...
class HttpHandler(commands.Cog):
  ...
  async def webserver(self):
    ...
    # Add logging
    handler = logging.FileHandler(filename='http.log', encoding='utf-8', mode='a')
